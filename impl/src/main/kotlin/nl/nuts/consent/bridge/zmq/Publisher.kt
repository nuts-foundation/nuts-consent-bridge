...
class Publisher {
    ...
    fun addSubscription(subscription: Subscription) {
        ...
        val subscriber = StateChangeSubscriber(subscription.topicId, context, stateChangeListenerFactory.createInstance(subscription.offset))
     ...
     */
    private class StateChangeSubscriber(val topic:String, val context: ZContext, val listener: StateChangeListener<ConsentRequestState>) {
        private val logger: Logger = LoggerFactory.getLogger(this::class.java)
        ...
        fun start() {
            ...
            listener.onProduced {
                try {
                    responseSocket!!.send("$topic:ConsentRequestState:${it.state.data}:produced")
            ...
            listener.onConsumed {
                try {
                    responseSocket!!.send("$topic:ConsentRequestState:${it.state.data}:consumed")
                } catch (e:ZMQException) {
                    logger.error("StateChangeSubscriber failed to publish due to error: ${e.message}")
                    stop()
                }
            ...
            logger.info("Started StateChangeSubscriber with topicId: $topic")
        }
        ...
        fun stop() : Boolean {
            ...
            logger.info("Stopped StateChangeSubscriber with topicId: $topic")
